...
public class Robot extends TimedRobot {
  ...
  public void autonomousPeriodic() {
    ...
    switch (autoStage) {
      case 0: {
        if (timer.get() > 0.5) {
          shooter.testVelocity(shooter.getXDistanceFromCenterOfHub(
              NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.00)));
          autoStage = 1;
        }
        // if(timer.get() > 1.0){
      ...
      case 1: {
        if (timer.get() > 2.5) {
          ...
          if (shooter.hoodMotor.getEncoder().getPosition() < ((90.0 - (Math.toDegrees(shooter.calculatedAngle))))) {
            shooter.hoodMotor.set(0.1);
          } else {
            shooter.hoodMotor.set(0.0);
            // shooter.hoodMotor.getPIDController().setReference(Math.toDegrees(shooter.calculatedAngle),
            // ControlType.kPosition);
            if (shooter.masterShooterMotor.getSelectedSensorVelocity() > shooter
                .shooterWheelLinearVelocityToMotorVelocity(shooter.calculatedVelocity) - 50.0) {
      ...
      case 2: {
        ...
        if (timer.get() > 10.0) {
          ...
          // shooter.testVelocity(shooter.getXDistanceFromCenterOfHub(NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.00))));
          shooter.testVelocity(shooter.getXDistanceFromCenterOfHub(
              NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.00)));
          autoStage = 3;
        }
