...
public class Shooter {
    ...
    private static float angleError = -1.23f; // was 0.04
    ...
    public static float slopeOfLine(float velocity, float possibleAngle, float x) {
        ...
        return  (float) (((y - x) / (((-1.0f) * ((-1.0f) * velocity * Math.sin(possibleAngle) - Math.sqrt(Math.pow(velocity, 2)
    ...
    public static float getXDistanceFromCenterOfHub(double verticalAngle) {
        return (float) (((y - cameraHeight) / Math.tan(Math.toRadians(verticalAngle + cameraAngle - angleError))) + distanceFromTapeToCenterOfHub + horizontalDistanceFromLimeLightToShooter);
    }
    ...
    public static float shooterWheelLinearVelocityToMotorVelocity(double projectileVelocity){
        return (float) (2.1f * (projectileVelocity * (1.0f/(4.0f*Math.PI)) * (1.0f/1.21) * 2048.0f * (1.0f/10.0f)));
    ...
    public void testingShooter(){
        ...
        if(shooting){
            if(shooterTimer.get() > 0.25){
                ...
                if((masterShooterMotor.getSelectedSensorVelocity() > shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity) - 25.0) && setHoodYet){
                    canShoot = true;
                } else {
                    canShoot = false;
                }
